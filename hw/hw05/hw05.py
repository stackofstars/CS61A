...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    attempts = []
    def withdraw(withdraw_balance, enter_pass):
        nonlocal password
        nonlocal balance
        if len(attempts) >= 3:
            return 'Too many incorrect attempts. Attempts: ' + str(attempts)
        if enter_pass == password and balance > withdraw_balance:
            balance -= withdraw_balance
            return balance
        elif enter_pass == password and withdraw_balance > balance:
            return 'Insufficient funds'
        else:
            attempts.append(enter_pass)
