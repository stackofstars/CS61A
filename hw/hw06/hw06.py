...
class VendingMachine:
    ...
    def vend(self):
        if self.funds >= self.price and self.stock > 0:
            self.stock -= 1
...
def is_bst(t):
    ...
    def bst_min(t):
        if t.is_leaf():
            ...
            return min(bst_max(t.branches[0]), t.label)
...
def store_digits(n):
    ...
    """
    digits_two = Link(n%10)
    while n > 0:
        n = n // 10
        if n > 0:
            digits_two = Link(n%10, digits_two)
    return digits_two
...
def path_yielder(t, value):
    ...
    for branch in t.branches:
        for result in path_yielder(branch, value):
