...
def g_iter(n):
    ...
    if n <= 3:
        ...
        while i < n-3:
            total = next + 2*curr + 3*prev
            prev, curr, next = curr, next, total
            i += 1
    return total
...
def missing_digits(n):
    ...
    if n < 10:
        ...
        return missing_digits(n // 10) + (last - second_last - 1)
...
def count_change(total):
    ...
    def change_helper(total,coin):
        ...
        if coin == 0:
            ...
            return change_helper(total-coin, coin) + change_helper(total, find_biggest_coin(coin , 1))

    def find_biggest_coin(total, x):
        if total == 1:
            return 0
        if x < total:
            return find_biggest_coin(total, 2*x)
        else:
            return (x/2)

    return change_helper(total, find_biggest_coin(total, 1))
