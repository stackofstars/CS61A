...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        lst = expr.rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(oper, lst, env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    """
    # BEGIN PROBLEM 7
    #print("DEBUG:", expressions.first, expressions, expressions.rest)
    #print("DEBUG:", expressions.rest == nil)
    if expressions == nil:
        return None
    else:
        value = None
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        updated_expr = Pair(expressions.first.rest, expressions.rest)
        env.define(expressions.first.first, do_lambda_form(updated_expr, env))
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        return '#f'
    while expressions.rest != nil:
        if is_true_primitive(scheme_eval(expressions.first, env)) or expressions == 0:
            return scheme_eval(expressions.first, env)
        expressions = expressions.rest
    return scheme_eval(expressions.first, env)
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            test_two = eval_all(clause.rest, env)
            if test_two == None:
                return test
            else:
                return test_two
