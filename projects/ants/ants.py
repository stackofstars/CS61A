...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if isinstance(place.ant, ContainerAnt) == True and place.ant.can_contain(self) == True:
                self.place = place
                place.ant.contain_ant(self)
            elif isinstance(place.ant, ContainerAnt) == False and isinstance(self, ContainerAnt) == True:
                self.place = place
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        near_bee = None
        if curr_loc != None:
            while curr_loc is not beehive:
                near_bee = rANTdom_else_none(curr_loc.bees)
                curr_loc = curr_loc.entrance
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        if isinstance(other, ContainerAnt) == False and self.contained_ant == None:
            return True
        else:
            return False
        # END Problem 9
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_watersafe == False:
            self.insect = insect.reduce_armor(insect.armor)
        # END Problem 11
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        self.index = QueenAnt.queen_index
        QueenAnt.queen_index = 1
    ...
    def action(self, gamestate):
        ...
        if self.index > 0:
            ...
            curr_loc = self.place.exit
            while curr_loc != None:
                if curr_loc.ant != None and curr_loc.ant.damage_doubled == False:
                    curr_loc.ant.damage *= 2
                    curr_loc.ant.damage_doubled = True
                if isinstance(curr_loc.ant, ContainerAnt):
                    guarded_ant = curr_loc.ant.contained_ant
                    if guarded_ant != None and guarded_ant.damage_doubled == False:
                        guarded_ant.damage *= 2
                        guarded_ant.damage_doubled = True
                curr_loc = curr_loc.exit
        # END Problem 13
...
def apply_status(status, bee, length):
    ...
    def status_help(gamestate):
        nonlocal length
        print("DEBUG:", "length:", length)
        print("DEBUG:", bee.is_scared)
        length = length - 1
        if length >= 0:
            return status(orig_action, bee)(gamestate)
        else:
            return orig_action(gamestate)
    bee.action = status_help
    # END Problem EC
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target:
            if target.has_been_scared == False:
